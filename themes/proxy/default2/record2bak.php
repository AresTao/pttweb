	...
	var num;
      ...
      $.ajax
       ({
           ...
           data: { sid: "1" },
		   dataType: "json",
           url: "./?ajax=server_getRecordsByOperator2&page=record2&najax=1&operatorId=<?php echo SessionManager::getInstance()->getLoginId();?>&recordType=1",
         ...
         $(function () {
              ...
              InitTable(0);    //Load事件，初始化表格数据，页面索引为0（第一页）
                ...
                function InitTable(pageIndex) {                                  
                    $.ajax({   
                        type: "POST",  
                        url: './?ajax=server_getRecordsByOperator2&page=record2&operatorId=<?php echo SessionManager::getInstance()->getLoginId();?>&recordType=1',      //提交到一般处理程序请求数据   
  ...
  <div class="filter">
    <form action="" method="post">
     ...
     <select name="recordType" id="recordlist">
                  <option value="4">从一级代理商购入</option>
                  <option value="5">向企业用户划拨</option>
     </select>
     </div>
  ...
  <div id="manager" class="mainBox" style="height:auto!important;height:550px;min-height:550px;">
   ...
   <form action="" method="post">
	...
	function jq_record_search(){
				...
				$.post(
				"./?ajax=server_record_search&sid=1&operatorId=<?php echo SessionManager::getInstance()->getLoginId();?>&type=2",
