<script>
	...
	var num;
		 ...
		 $.ajax
		     ...
		     data: { sid: "1" },
		     dataType: "json",
		     url: "./?ajax=server_getRecordsByOperator2&page=record1&najax=1&operatorId=<?php echo SessionManager::getInstance()->getLoginId();?>&type=1&recordType="+recordType,
         ...
         function InitTable(pageIndex, recordType) {                                  
                    $.ajax({   
                        type: "POST",  
                        url: './?ajax=server_getRecordsByOperator2&page=record1&operatorId=<?php echo SessionManager::getInstance()->getLoginId();?>&type=1&recordType='+recordType,      //提交到一般处理程序请求数据   
  ...
  <div class="filter">
    <form action="" method="post">
     ...
     <select name="recordType" id="recordlist">
                  <option value="1">从上级代理商购入</option>
                  <option value="2">向企业用户划拨</option>
     </select>
     </div>
     <div id = "search" >
     <label style='font-size:15px;padding: 5px 5px 5px 2px;'>搜索类型</label> 
     <select name="cat_id" id="catlist">
		  <option value="1">上级代理商编号</option>
		  <option value="2">上级代理商姓名</option>
     </select>
   ...
   <form action="" method="post">
	...
	<script type="text/javascript">
			...
			function jq_record_search(){
				...
				$.post(
				"./?ajax=server_record_searchByOperator2&sid=1&operatorId=<?php echo SessionManager::getInstance()->getLoginId();?>&type=1",
