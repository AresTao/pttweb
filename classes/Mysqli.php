...
class MysqlManager{
	...
	private function __construct($db_config=array()){
		...
		$this->port = $db_config['DB_PORT'] ? $db_config['DB_PORT'] : '3306';
		$this->user = $db_config['DB_USER'] ? $db_config['DB_USER'] : 'pttweb';
		$this->pass = $db_config['DB_PWD'] ? $db_config['DB_PWD'] : 'pttweb';
		$this->db = $db_config['DB_NAME'] ? $db_config['DB_NAME'] : 'pttweb';
		$this->charset=$db_config['DB_CHARSET'] ? $db_config['DB_CHARSET']: 'utf8';
		//连接数据库
		$this->db_connect();
		//选择数据库
		//$this->db_usedb();
	...
	public static function getInstance(){
		if(self::$dbcon==false){
			self::$dbcon=new self(SettingsManager::getInstance()->getMysqlConfig());
		}
		return self::$dbcon;
	}
	//执行sql语句的方法
